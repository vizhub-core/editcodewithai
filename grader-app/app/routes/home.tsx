...
export function meta({}: Route.MetaArgs) {
  return [
    ...
    { name: "description", content: "Grade AI-generated visualizations" },
  ];
}
...
export default function Home() {
  ...
  const fetchResults = async () => {
    try {
      const response = await fetch("benchmarks/results/results.csv");
      const csv = await response.text();
      const [header, ...lines] = csv.trim().split("\n");
      const parsedResults = lines.map(line => {
        const [challenge, model, passFail, grade, aesthetics, reviewedBy, reviewedAt, notes] = 
        ...
        return {
          ...
          aesthetics: aesthetics ? Number(aesthetics) : undefined,
          reviewedBy,
          reviewedAt,
  ...
  const handleGradeChange = (index: number, type: "technical" | "aesthetics", value: number) => {
    setResults(prev => {
      const newResults = [...prev];
      newResults[index] = { ...newResults[index], [type]: value };
      return newResults;
    });
    saveResults();
  };
  ...
  return (
    <div className="container mx-auto p-4">
      <div className="header">
        <h1 className="text-2xl font-bold">Grade Result Player</h1>
        <input
          type="text"
