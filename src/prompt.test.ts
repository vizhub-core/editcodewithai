...
describe("prompt", () => {
  describe("assembleFullPrompt", () => {
    it("should combine task, files context, and formatting instructions", () => {
      ...
      expect(result).toContain("Suggest changes to the original files using this exact format");
    });
    ...
    it("should maintain the correct order of sections", () => {
      ...
      const filesIndex = sections.findIndex(s => s.includes("## Original Files"));
      const formatIndex = sections.findIndex(s => s.includes("## Formatting Instructions"));
